...
- MCP는 AI 에이전트가 외부 도구와 연결될 때 쓰는 표준 인터페이스 규약입니다.
- 쉽게 말해 에이전트용 USB-C처럼, 여러 도구를 비슷한 방식으로 붙이게 해주는 역할에 가깝습니다.
- Codex, Claude Code, Copilot 같은 에이전트 도구를 더 확장성 있게 쓰고 싶다면 지금 가장 먼저 이해할 개념 중 하나입니다.
