...
![OpenClaw ?? ??](/assets/images/posts/etc/openclaw-guide/logo.png)
...
- 공식 사이트: [OpenClaw 공식 사이트](https://openclaw.ai)
- 다운로드/접속 경로: [OpenClaw 릴리스 페이지](https://github.com/openclaw/openclaw/releases)
- 공식 문서: [OpenClaw Getting Started](https://docs.openclaw.ai/start/getting-started)
...
대표적으로는 PowerShell 설치 스크립트와 npm 전역 설치를 많이 씁니다.
...
보통은 설치 스크립트나 npm 전역 설치 중 하나를 고르면 됩니다.
...
대표적으로는 설치 스크립트와 npm 전역 설치를 많이 씁니다.
...
OpenClaw는 처음부터 여러 채널을 붙이기보다, 자주 쓰는 메신저 하나로 좁혀서 온보딩하는 편이 좋습니다. 모델 공급자와 채널 연동이 동시에 얽히기 때문에, 한 경로만 먼저 안정화해 두면 이후 확장이 훨씬 편해집니다.
