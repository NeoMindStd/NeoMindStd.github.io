---
title: "방송·오디오·영상 도구 허브"
permalink: /tools/creator-media/
layout: single
excerpt: "OBS, XSplit, GoldWave, Cakewalk, DaVinci Resolve처럼 방송과 편집에 자주 쓰는 도구를 모은 허브입니다."
description: "녹화, 스트리밍, 오디오 편집, DAW, 영상 편집 툴을 빠르게 비교하고 시작할 수 있게 정리한 NeoMind Tools 허브입니다."
---

- - -

 - [Tools 허브 전체 보기](/tools/)

- - -

# 방송·오디오·영상 도구 허브

녹화, 스트리밍, 오디오 편집, 작곡, 영상 편집처럼 크리에이터 작업에 가까운 도구만 모았습니다.

### 대표 도구 카드

<div class="nm-tools-pick-grid">
  <article class="nm-tools-pick-card">
    <h4><a href="/tools/obs-studio-guide/">OBS Studio</a></h4>
    <p>무료로 녹화와 송출을 모두 시작할 수 있어서 크리에이터 워크플로우의 첫 도구로 가장 무난합니다.</p>
    <p class="nm-tools-badges">
      <span class="nm-tools-badge">난이도: 입문</span>
      <span class="nm-tools-badge">OS: Win/macOS/Linux</span>
      <span class="nm-tools-badge">추천: 방송·녹화</span>
    </p>
  </article>
  <article class="nm-tools-pick-card">
    <h4><a href="/tools/davinci-resolve-guide/">DaVinci Resolve</a></h4>
    <p>컷 편집부터 컬러와 렌더까지 한 도구에서 처리할 수 있어 영상 제작 파이프라인에 강합니다.</p>
    <p class="nm-tools-badges">
      <span class="nm-tools-badge">난이도: 중급</span>
      <span class="nm-tools-badge">OS: Win/macOS/Linux</span>
      <span class="nm-tools-badge">추천: 영상 편집</span>
    </p>
  </article>
  <article class="nm-tools-pick-card">
    <h4><a href="/tools/cakewalk-guide/">Cakewalk</a></h4>
    <p>멀티트랙과 MIDI 기반의 음악 제작을 해보고 싶을 때 접근성이 좋은 DAW입니다.</p>
    <p class="nm-tools-badges">
      <span class="nm-tools-badge">난이도: 입문~중급</span>
      <span class="nm-tools-badge">OS: Windows</span>
      <span class="nm-tools-badge">추천: 작곡·믹싱</span>
    </p>
  </article>
</div>

### 작업 흐름별 빠른 선택표

| 목적 | 먼저 볼 글 | 잘 맞는 상황 | 다음 단계 |
|:--|:--|:--|:--|
| 화면 녹화와 방송 송출을 시작할 때 | [OBS 설치와 사용법](/tools/obs-studio-guide/) | 무료로 녹화와 방송을 동시에 시작하고 싶을 때 | XSplit, DaVinci Resolve |
| 방송 장면 구성과 송출 UI를 더 쉽게 다루고 싶을 때 | [XSplit 설치와 사용법](/tools/xsplit-broadcaster-guide/) | GUI 중심 방송 환경을 선호할 때 | OBS |
| 빠른 오디오 편집이 필요할 때 | [GoldWave 설치와 사용법](/tools/goldwave-guide/) | 컷 편집, 잡음 제거, 간단한 오디오 작업이 많을 때 | Cakewalk |
| 음악 제작과 DAW가 필요할 때 | [Cakewalk 설치와 사용법](/tools/cakewalk-guide/) | 멀티트랙 작업과 MIDI를 다뤄야 할 때 | GoldWave |
| 본격적인 영상 편집과 렌더가 필요할 때 | [DaVinci Resolve 설치와 사용법](/tools/davinci-resolve-guide/) | 컷 편집, 컬러, 렌더까지 한 툴에서 하고 싶을 때 | OBS |

### 추천 순서

1. 녹화나 송출이 먼저면 `OBS`를 시작점으로 잡는 편이 가장 무난합니다.
2. 오디오만 빠르게 손보고 싶다면 `GoldWave`, 음악 제작까지 갈 거면 `Cakewalk`로 이어서 보면 됩니다.
3. 영상 편집까지 필요한 경우엔 마지막에 `DaVinci Resolve`로 넘어가면 흐름이 잘 이어집니다.

### 방송·송출

 - [OBS 설치와 사용법: 공식 다운로드부터 첫 녹화와 송출까지](/tools/obs-studio-guide/)
 - [XSplit 설치와 사용법: 공식 다운로드부터 기본 방송 화면 구성까지](/tools/xsplit-broadcaster-guide/)

### 오디오 편집·작곡

 - [GoldWave 설치와 사용법: 공식 다운로드부터 기본 오디오 편집까지](/tools/goldwave-guide/)
 - [Cakewalk 설치와 사용법: 공식 사이트 기준으로 오디오 제작 시작하기](/tools/cakewalk-guide/)

### 영상 편집

 - [DaVinci Resolve 설치와 사용법: 공식 다운로드부터 첫 렌더까지](/tools/davinci-resolve-guide/)

- - -

 - [Tools 허브 전체 보기](/tools/)

- - -
