...
- [ ] MCP가 모델 이름이 아니라 연결 규약이라는 점을 이해했는가
- [ ] Host / Client / Server / Tool / Resource / Prompt를 구분할 수 있는가
- [ ] MCP가 도구 연결을 쉽게 하지만 보안 설계까지 대신해주지는 않는다는 점을 알고 있는가
- [ ] 읽기/쓰기 권한 분리와 실행 범위 제한이 중요하다는 점을 알고 있는가
