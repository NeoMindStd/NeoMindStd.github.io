...
- [ ] `AI`와 `머신러닝`을 같은 말로 섞어 쓰지 않는가
- [ ] `딥러닝`이 머신러닝의 하위 개념이라는 점을 알고 있는가
- [ ] `강화학습`이 정답 맞히기보다 보상 최적화에 가깝다는 점을 이해했는가
- [ ] `유전알고리즘`이 학습이라기보다 탐색/최적화에 강하다는 점을 이해했는가
- [ ] LLM과 에이전트가 이 기초 위에 쌓인 응용 계층이라는 점을 이해했는가
