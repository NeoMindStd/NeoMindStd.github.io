...
대표 경로는 두 가지입니다. 가장 빠른 방법은 브라우저에서 바로 들어가는 것이고, 데스크톱 흐름을 원하면 Windows 앱 가이드를 따라가면 됩니다.

```text
https://chatgpt.com/codex
```

Windows 앱 흐름은 공식 문서의 `Codex on Windows` 페이지에서 설치와 WSL 연동 단계를 따라가면 됩니다.

## macOS

macOS에서는 브라우저 접속이나 데스크톱 앱 경로를 통해 진입하는 방식이 자연스럽습니다. 로컬 에디터와 연결하려면 Codex IDE 문서를 같이 보는 편이 좋습니다.

대표 경로는 브라우저 접속과 macOS 앱 다운로드입니다. 브라우저로 바로 들어가도 되고, 앱 중심으로 쓸 거면 OpenAI의 `Download for macOS` 경로를 쓰면 됩니다.
...
대표 경로는 브라우저 접속이고, 로컬 편집기와 함께 쓸 때는 IDE 확장 문서를 같이 보는 방식이 가장 단순합니다.
